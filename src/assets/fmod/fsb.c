...
igiari_fmod_fsb_sample_chunk* igiari_fmod_fsb_impl_ReadChunkFromPtr(igiari_fmod_fsb_sample_metadata* metadata, char* ptr, size_t* bytes_read) {
    igiari_fmod_fsb_sample_chunk* chunk = malloc(sizeof(igiari_fmod_fsb_sample_chunk));
...
igiari_fmod_fsb_sample_metadata* igiari_fmod_fsb_impl_ReadMetadataFromPtr(char* ptr) {
    igiari_fmod_fsb_sample_metadata* metadata = (igiari_fmod_fsb_sample_metadata*)malloc(sizeof(igiari_fmod_fsb_sample_metadata));;
...
igiari_fmod_fsb_header* igiari_fmod_fsb_impl_ReadHeaderFromPtr(char* ptr, size_t* bytes_read) {
    igiari_fmod_fsb_header* header = (igiari_fmod_fsb_header*)malloc(sizeof(igiari_fmod_fsb_header));
    ...
    header->samples = (igiari_fmod_fsb_sample_metadata**)malloc(sizeof(igiari_fmod_fsb_sample_metadata*));
    int sample_count = 0;
    for (int i = 0; i < header->num_samples; i++)
    {
        ...
        metadata->chunks = (igiari_fmod_fsb_sample_chunk**)malloc(sizeof(igiari_fmod_fsb_sample_chunk*));
        int chunk_count = 0;
        
        igiari_fmod_fsb_sample_chunk* next_chunk = NULL;
        do
        {
            size_t bytes_read = 0;
...
igiari_fmod_fsb* igiari_fmod_fsb_ReadFromPtr(char* ptr) {
    ...
    for (int i = 0; i < fsb->header->sample_count; i++)
    {
       ...
       //printf("[igiari, fsb] Reading %i samples\n", fsb->header->sample_count);
        ...
        igiari_fmod_fsb_sample* sample = (igiari_fmod_fsb_sample*)malloc(sizeof(igiari_fmod_fsb_sample));
...
void igiari_fmod_fsb_FreeChunk(igiari_fmod_fsb_sample_chunk* chunk) {
    free(chunk);
}

void igiari_fmod_fsb_FreeMetadata(igiari_fmod_fsb_sample_metadata* metadata) {
    for (int i = 0; i < metadata->chunk_count; i++) {
        igiari_fmod_fsb_FreeChunk(metadata->chunks[i]);
    }
    free(metadata);
}

void igiari_fmod_fsb_FreeSample(igiari_fmod_fsb_sample* sample) {
    free(sample->sample_bytes);
    free(sample);
}

void igiari_fmod_fsb_Free(igiari_fmod_fsb* fsb) {
    for (int i = 0; i < fsb->header->sample_count; i++) {
        igiari_fmod_fsb_FreeMetadata(fsb->header->samples[i]);
    }
    for (int i = 0; i < fsb->sample_count; i++) {
        igiari_fmod_fsb_FreeSample(fsb->samples[i]);
    }
    free(fsb);
}